...
const data = paccounts.data?.filter((account) => new BN(account.account.id|| 0).toNumber()  === Number(params.id));
const leaderboard = data?.sort((a:any, b:any) => b.account.points - a.account.points);
const myData =  data?.find((account) => new BN(account.account.id|| 0).toNumber()  == Number(params.id));
const myRank = (leaderboard?.findIndex((account) => new BN(account.account.id|| 0).toNumber()  == Number(params.id)) ?? -1) + 1;
const provider = useAnchorProvider();

const myv = accounts.data?.find((account) => account.account.id.toNumber() === Number(params.id));
...
const creator = accounts.data?.find((account) => account.account.owner.toString()  == provider.wallet.publicKey.toString());
  return (
      ...
      {creator?
        ...
        <div className="relative h-[200px] sm:h-[250px]">
          {leaderboard?.slice(0, 3).map((player, index) => (
            <div key={new BN(player.account.id|| 0).toNumber()} className={`absolute ${index === 1 ? 'left-1/2 bottom-8 transform -translate-x-1/2' : index === 0 ? 'left-0 sm:left-1/4 bottom-0 transform sm:-translate-x-1/2' : 'right-0 sm:left-3/4 bottom-0 transform sm:-translate-x-1/2'} flex flex-col items-center`}>
              <div className="relative mb-2">
                <div className={`w-16 h-16 sm:w-20 sm:h-20 ${index === 1 ? 'sm:w-24 sm:h-24' : ''} rounded-lg overflow-hidden`}>
                  <Image
                  src={(index === 0) ? "https://assets.promptbase.com/DALLE_IMAGES%2F3X1SXaf7riVzZ4fmeD70dlwbQvD3%2Fresized%2F1679336685019_800x800.webp?alt=media&token=d2d7f2fa-d397-43c8-8f52-8282e5728752" : index === 1 ? "https://img.freepik.com/free-vector/hand-drawn-nft-style-ape-illustration_23-2149622021.jpg" : "https://i.pinimg.com/474x/aa/f8/59/aaf859dde8eb62bd0c382bb047a53ce7.jpg"} 
        ...
        <div className="bg-gray-900/50 rounded-full py-2 px-4 text-center text-sm text-gray-400">
          <span>You earned <span className="text-[#ff6b95]">{myData?.account.points.toNumber()}</span> today and are ranked <span> {myRank}</span> out of <span className="text-white">{leaderboard?.length}</span> users</span>
        </div>
          ...
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody>
              {leaderboard?.slice(3).map((player, index) => (
                <tr key={player.account.id.toNumber()} className="border-b border-gray-800">
                  ...
                  <td className="p-2 sm:p-4">{player.account.user.toString()}</td>
