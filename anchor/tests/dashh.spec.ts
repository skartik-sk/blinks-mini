...
import IDL from '../target/idl/dashh.json';

const campaign_address = new PublicKey(
...
describe("dashh", () => {
  ...
  it("participate in campaign", async () => {
    await dashhProgram.methods
      .createParticipent(id, dashhKeypair.publicKey)
      .rpc();

    const [participantAddress] = PublicKey.findProgramAddressSync(
      [id.toArrayLike(Buffer, "le", 8), dashhKeypair.publicKey.toBuffer()],
      campaign_address,
    );

    const participant =
      await dashhProgram.account.participent.fetch(participantAddress);
  ...
  it("updates in participant", async () => {
    await dashhProgram.methods
      .updatedParticipent(id, dashhKeypair.publicKey, new anchor.BN(100))
      .rpc();
